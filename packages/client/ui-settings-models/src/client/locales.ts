...
export const en = {
  ...
  awsProfileHint: 'Amazon Bedrock only: the shared-configuration profile, SSO included, that requests authenticate as. Leave blank to use an API key or the ambient AWS credentials.',
  awsProfileUnset: 'Not used',
  awsRegion: 'AWS region',
  awsRegionDefault: 'Endpoint default',
  models: 'Models',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  awsProfile: 'AWS profile',
  awsProfileHint: '仅用于 Amazon Bedrock：请求据以认证的 AWS 共享配置 profile，SSO profile 也在其中。留空则使用 API 密钥或环境中的 AWS 凭据。',
  awsProfileUnset: '未使用',
  awsRegion: 'AWS region',
  awsRegionDefault: '端点默认',
  models: '模型目录',
