...
- `ctx.llm.listConfigurableProviders(): LlmConfigurableProvider[]` 按声明顺序列出可配置提供方目录；配置界面将其与 `listProviders()` 合并，为每个条目标注存活或休眠。条目可携带 `declared`，表示拥有该路由的适配器是否只因配置点名才知道它。只有适配器能回答这一点：该字段缺失时，只表示该适配器不区分这两种来源，不能据此判断路由是否随产品交付。条目还可携带 `nativeAuthFields`：相对于其 `settingsPath` 的档案字段名，表示该路由接受哪些字段用于提供方原生认证，而不是 API 密钥。界面在该路由的卡片上恰好渲染这些字段，因此某个提供方特有的凭据控件不会出现在其他提供方的卡片上；只有适配器能回答这一点，因为 settings schema 本身对该 section 内的每条路由共用同一份档案形状。
