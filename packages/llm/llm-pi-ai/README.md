        ...
        modelOverrides:
          deepseek-v4-pro:
            reasoningEfforts:
              off:
              high: high
      # Catalog route authenticating through the AWS credential chain: the
      # named profile may be an SSO profile, and no key is stored anywhere.
      amazon-bedrock:
        awsProfile: sso-prod
        awsRegion: eu-west-1
...
### AWS credential chain routes

`awsProfile` names the AWS shared-configuration profile a Bedrock route authenticates as, an SSO profile included. It reaches the request as the `AWS_PROFILE` provider environment value, so the AWS SDK's own credential chain resolves it — an SSO profile uses the token `aws sso login` cached and refreshes it without the harness reading either — and that value is also what makes the route report itself configured, so no credential record and no `apiKeyEnv` reference are involved. `awsRegion` rides beside it as `AWS_REGION`, and naming it is what reaches a region other than the installed catalog endpoint's `us-east-1`: the Bedrock client reads the region from the request environment and from the model's endpoint, never from the named profile's own `region` setting. It is usable beside a bearer token or the ambient credential chain, neither of which carries a region.

Both fields are refused on a route whose models all speak some other protocol, where nothing would read them, and `awsProfile` is refused beside `apiKeyEnv`, whose resolved key becomes the Bedrock bearer token and takes the request off SigV4 entirely. The same fact reaches configuration surfaces through the directory: a route the Bedrock client serves carries `nativeAuthFields: [awsProfile, awsRegion]`, and every other route carries none, so a page renders those two inputs on the Bedrock card alone. A route whose stored credential record already carries `AWS_PROFILE` — what pi-ai's own sign-in writes — needs neither field; configuring them keeps the choice in `settings.yaml` instead of the credential store.
