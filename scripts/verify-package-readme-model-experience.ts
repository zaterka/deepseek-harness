...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/core/dev-mode-pipeline': { kind: 'none', reason: 'The service only stores and resolves configuration; the calling agent decides what reaches a model request.' },
  ...
  'packages/client/ui-settings-dev-mode-pipeline': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  ...
  'packages/bundle/tui': { kind: 'none', reason: 'The interactive client submits typed lines as ordinary user messages and renders session events on the terminal; prompts and tools belong to the composed base and tui bundles.' },
  ...
  'packages/web/web-search-duckduckgo': { kind: 'indirect', reason: 'The provider backend delegates model rendering to dsh-tool-web.' },
